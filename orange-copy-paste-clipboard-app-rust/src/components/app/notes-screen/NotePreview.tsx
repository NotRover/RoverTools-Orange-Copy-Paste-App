...
function clipLabel(node: ClipEmbedNode, entries: ClipboardEntry[]): string {
  ...
  return entry.type === "image"
    ...
    : entry.type === "file"
      ...
      : truncateText(
          (entry.type === "html" ? entry.content.replace(/<[^>]*>/g, "") : entry.content)
            .replace(/\s+/g, " ").trim(),
          28,
        ) || "Clip";
}
...
function InlineEl({ node, entries }: { node: InlineNode; entries: ClipboardEntry[] }) {
  switch (node.type) {
    case "text": {
    ...
    case "clip_embed": {
      ...
      return (
        <span className={`ns-preview-embed-chip${!entry && n.id ? " ns-preview-embed-chip--missing" : ""}`}>
          {clipLabel(n, entries)}
        </span>
      );
    }
...
function InlineList({ nodes, entries }: { nodes: InlineNode[]; entries: ClipboardEntry[] }) {
  return <>{nodes.map((n, i) => <InlineEl key={i} node={n} entries={entries} />)}</>;
}

// ── Block renderer ────────────────────────────────────────────────────────

function BlockEl({ node, entries }: { node: BlockNode; entries: ClipboardEntry[] }) {
  switch (node.type) {
    case "p":
      return node.children.length === 0
        ? <div className="ns-pv-line ns-pv-line--empty" />
        : <div className="ns-pv-line"><InlineList nodes={node.children} entries={entries} /></div>;
    case "h1":
      return <div className="ns-pv-line ns-pv-h1"><InlineList nodes={node.children} entries={entries} /></div>;
    case "h2":
      return <div className="ns-pv-line ns-pv-h2"><InlineList nodes={node.children} entries={entries} /></div>;
    case "h3":
      return <div className="ns-pv-line ns-pv-h3"><InlineList nodes={node.children} entries={entries} /></div>;
    case "ul":
      return (
        <ul className="ns-pv-ul">
          {node.items.map((item, i) => (
            <li key={i} className="ns-pv-li"><InlineList nodes={item} entries={entries} /></li>
          ))}
        </ul>
      );
    case "ol":
    ...
    case "bq":
      return <div className="ns-pv-bq"><InlineList nodes={node.children} entries={entries} /></div>;
    case "todo":
      return (
        <div className={`ns-pv-todo${node.checked ? " ns-pv-todo--checked" : ""}`}>
          <span className="ns-pv-todo-check" />
          <InlineList nodes={node.children} entries={entries} />
        </div>
      );
    case "code":
      return <pre className="ns-pv-code-block"><InlineList nodes={node.children} entries={entries} /></pre>;
    case "hr":
