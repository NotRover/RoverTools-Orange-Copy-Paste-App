...
> **Scope:** Cross-system reference covering both submodules.
> For deep internals see:
...
- WebSocket: `GET /ws?token=<access_token>`
- Refresh: `POST /api/v1/auth/refresh` with rotating refresh token
...
| `device:online` | A device in the user's account connected | `{ device_id, device_name }` |
...
Sign-up flow:
  User → Settings → Cloud Sync → Create Account
  App → POST /auth/register { email, password, display_name }
  App → POST /auth/login (immediately after)
  Server returns kdf_salt
  App derives UMK = Argon2id(password, kdf_salt)  [in memory only]
  App generates X25519 device keypair
  App → POST /auth/keys/register { identity_pubkey, device_pubkey }
  App stores refresh_token in OS keychain
  Sync begins

Subsequent launch (account exists):
  App reads refresh_token from OS keychain
  App → POST /auth/refresh { refresh_token, device_id }
  App prompts password re-entry to re-derive UMK (never persisted)
  Sync client connects to WebSocket

New device (same account):
  Same sign-up flow → new device_id issued
  Server notifies existing devices via WS: "new device pending key approval"
  User approves on an existing device:
    Existing device: fetches new device's device_pubkey
    Computes shared_secret = X25519(my_privkey, new_device_pubkey)
    wrapped_umk = AES-256-GCM(shared_secret, UMK)
    → POST /auth/devices/{new_device_id}/key-wrap { wrapped_umk }
...
On app startup (sync enabled, network available):
  1. Authenticate (refresh token → access token)
...
```
              ...
              ┌──────────┼──────────┐
              ▼          ▼          ▼
         uvicorn(1)  uvicorn(2)  uvicorn(3)   (uvicorn --workers 4)
              └──────────┬──────────┘
                         │
              ┌──────────┼──────────┐
              ▼          ▼          ▼
         ...
         (same host)  (same host) (same host)
                         │
                    ┌────▼─────┐
                    │ R2 Bucket│  (Cloudflare R2 — zero egress)
                    └──────────┘
```

### Scale-out path (10k+ users)
- API tier: stateless uvicorn fleet behind load balancer (no code changes)
- Database: managed PostgreSQL (Supabase / Neon / RDS)
- Cache: managed Redis (Upstash / ElastiCache)
- Blob: R2 already external
- WebSocket fan-out: Redis pub/sub already multi-process safe
